...
Then all root packages must be opened by double-clicking on them.
...
(Note, currently this only works with hardcoded paths relative to the working directory at the time of execution, however we plan to change this later to allow thee file locations to be specified through a CLI)